...
export class TableRowBlock extends SlateBlock<"tr", {}> {
  ...
  constructor() {
    super("tr", {
      ...
      allowedChildren: [
        TableCellBlock.withId("td"),
        TableCellBlock.withId("th"),
      ],
      behaviours: [
        {
          ...
          when: ({ editor }) => {
            if (editor.selection && Range.isExpanded(editor.selection)) {
              ...
              const higherPoint = Point.isBefore(
                editor.selection.anchor,
                editor.selection.focus
              )
                ? editor.selection.anchor
                : editor.selection.focus
              const lowerPoint = Point.isBefore(
                editor.selection.anchor,
                editor.selection.focus
              )
                ? editor.selection.focus
  ...
  static withType(type: "td" | "th") {
    const block = new TableRowBlock()
    block.emptyBlock = {
      type: "tr",
      children: [
        new TableCellBlock(type).emptyBlock,
        new TableCellBlock(type).emptyBlock,
        new TableCellBlock(type).emptyBlock,
      ],
    }
  ...
  render(props: RenderElementProps<TableRowBlock>) {
    return vueJsxCompat("tr", props.attributes, props.children as any)
  }
}
