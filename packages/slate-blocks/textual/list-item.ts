...
export class ListItemBlock extends SlateBlock<"li", { level: number }> {
      ...
      behaviours: [
        {
          ...
          when: ({ editor, element, tree }) => {
            ...
            const isOnlyChild =
              tree.flatMap(([entry]) =>
                Element.isElement(entry)
                  ? entry.children.filter(c => ListItemBlock.assert(c))
                  : []
              ).length === 1
          ...
          target: ({ tree }) => {
            ...
            const path = reversedTree.find(([entry]) =>
              ListBlock.assert(entry)
            )?.[1]
            return path
          },
        },
        ...
        },
        {
          ...
          action: ({ editor, element }) => {
            Transforms.setNodes<ListItemElement>(editor, {
  ...
  render(props: RenderElementProps<ListItemBlock>): any {
    return vueJsxCompat(
      "li",
      {
        ...props.attributes,
        style: { paddingLeft: `${20 * props.element.level}px` },
      },
